...
Once enabled, agent sessions can be recorded from the agents' own session
stores — no agent configuration is touched.`,
	Args: cobra.NoArgs,
...
var auditDisableCmd = &cobra.Command{
	...
	Long: `Disable sets ops.enabled=false — no new sessions are recorded. The
database is left in place; re-enable with 'qvr audit enable'.`,
	Args: cobra.NoArgs,
	RunE: func(cmd *cobra.Command, args []string) error {
...
func setAuditEnabled(ctx context.Context, enabled bool) error {
	...
	// away since each capture path opens its own handle. (#144)
	if enabled {
	...
	if enabled {
		...
		printer.Hint("run `qvr audit discover` to back-fill from your agents' session stores")
	}
	return nil
}
